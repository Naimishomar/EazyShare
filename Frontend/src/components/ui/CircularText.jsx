...
const CircularText = ({
  ...
  return (
    <motion.div
      initial={{ rotate: 0 }}
      className={`mx-auto rounded-full w-[80px] h-[80px] ${changeBackground ? "text-black" : "text-white"} font-black text-center cursor-pointer origin-center ${className}`}
      animate={controls}
    ...
    >
      {letters.map((letter, i) => {
        ...
        return (
          <span
            key={i}
            className="absolute inline-block inset-0 text-[13px] font-medium transition-all duration-500 ease-[cubic-bezier(0,0,0,1)]"
